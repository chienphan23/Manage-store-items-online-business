...
/**
 * Servlet implementation class adminSuaHangHoaController
 */
@WebServlet("/adminSuaHangHoaController")
public class adminSuaHangHoaController extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public adminSuaHangHoaController() {
		super();
		...
		try {
			...
			for (FileItem fileItem : fileItems) {
				...
				} else// Neu la control
				{
					...
					if (tentk.equals("txtMaHang")) {
						response.getWriter().println(fileItem.getString());
						txtMaHang = fileItem.getString();
					}
			...
			try {
				...
				if (txtTenHang != null && txtGia != null && txtSoLuong != null && txtMaLoai != null
					...
					QLHHBo.SuaHang(Long.parseLong(txtMaHang), txtTenHang, Long.parseLong(txtGia),
							Long.parseLong(txtSoLuong), tenAnh1, Long.parseLong(txtMaLoai),
							Long.parseLong(txtKieuLoai));

					QLHHBo.xoaChiTietVoiMaHang(Long.parseLong(txtMaHang));

					for (long l : dsSize) {
						QLHHBo.ThemChiTietHang(Long.parseLong(txtMaHang), l);
