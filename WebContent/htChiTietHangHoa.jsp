...
<a href="dangKyController" class="btn btn-outline-dark text-white border border-white" style="display: flex;justify-content: center;align-items: center"><svg style="margin-right: 4px" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-box-arrow-in-right" viewBox="0 0 16 16">
  ...
  <path fill-rule="evenodd" d="M11.854 8.354a.5.5 0 0 0 0-.708l-3-3a.5.5 0 1 0-.708.708L10.293 7.5H1.5a.5.5 0 0 0 0 1h8.793l-2.147 2.146a.5.5 0 0 0 .708.708l3-3z"/>
	...
	<div class="row" style="margin: 20px 0">
		<%hangHoaBean h = (hangHoaBean) request.getAttribute("cthh");
		%>
		
		<div class="col-lg-5   hover-div">
			
			<img src="<%= h.getAnh() %>" class="img-thumbnail">
			
		
		</div>
		<div class="col-lg-7">
			<h2 style="font-weight: bold"><%= h.getTenHang() %></h2>
			<p style="font-size: 20px; font-weight: bold">Giá: <%= h.getGia() %> <span style="font-size: 12px">₫</span></p>
			<p>Ngày nhập hàng: <%=h.getNgayNhap() %></p>
			<p>Số lượng còn:  &nbsp <%= h.getSoLuong() %></p>
			
			<form action="gioController?maHang=<%=h.getMaHang() %>&tenHang=<%=h.getTenHang() %>&gia=<%=h.getGia() %>&soLuongTon=<%=h.getSoLuong() %>" method="post">
			<div style="margin-top: 20px"> Size: 
				<%ArrayList<chiTietHangHoaBean> dscthh = (ArrayList<chiTietHangHoaBean>) request.getAttribute("dsCTHH");
				int n = dscthh.size();
				for(int i = 0 ; i < n; i++ ){
				%>
				<input type="radio" class="btn-check" name="options" id="option<%= i + 1%>" autocomplete="off" value="<%=dscthh.get(i).getMaSize()%>" <%= (i == 0 ? "checked" : "") %> >
				<label class="btn btn-secondary" for="option<%= i + 1%>"><%= dscthh.get(i).getTenSize() %></label>
				
				<%} %>
				<p>Chọn size của bạn (Mặc định là size <%=dscthh.get(0).getTenSize() %>)</p>
			</div>

			
			<div style="text-align: center;margin-top: 20px">
			<%if(h.getSoLuong() != 0){ %>
			<button type="submit" class="btn btn-dark" style="width: 50%" > Thêm vào giỏ hàng</button>
			<%}else{ %>
			<a disable class="btn btn-secondary" style="width: 50%">Hết hàng</a>
			<%} %>
			</div>
			</form>
		</div>
